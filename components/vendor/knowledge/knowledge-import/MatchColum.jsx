...
//                     <td key={cellIndex} style={tdStyle} className='w-[220px] h-[30px]'>
...
//               ))}
//               <tr>
//                 <td className='pt-5 pb-2 font-semibold text-[18px] 2xl:text-[20px] w-[220px] h-[30px]'>Will be mapped with</td>
//               </tr>
//               <tr className=''>
//                 {selectedHeader?.map((header, index) => {
...
//                   return (
//                     <td key={index} className='w-[220px] h-[30px]'>
//                       <Select
//                         variant="bordered"
        ...
        <div>
          <table className='text-sm'>
            <thead className=''>
              <tr className='bg-[#ebeef2]'>
                {selectedHeader?.map((header, index) => (
                  <th key={index} style={thStyle} className='h-[30px] text-[18px] 2xl:text-[20px]'>{header}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {knowledgeData.questions.slice(1, 3).map((row, rowIndex) => (
                <tr key={rowIndex} className='text-[18px] 2xl:text-[20px]'>
                  {row.map((cell, cellIndex) => (
                    <td key={cellIndex} style={tdStyle} className='h-[30px]'>
                      {cell.length > 15 ? `${cell.substring(0, 15)}...` : cell}
                    </td>
                  ))}
                  {Array.from({ length: selectedHeader.length - row.length }).map((_, idx) => (
              ...
              <tr>
                <td className='pt-5 pb-2 font-semibold text-[18px] 2xl:text-[20px] h-[30px]'>Will be mapped with</td>
              </tr>
              <tr className=''>
                {selectedHeader?.map((header, index) => {
                  const currentSelectedValue = mappedIndexValue[index];
