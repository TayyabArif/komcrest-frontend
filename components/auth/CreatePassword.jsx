...
const CreatePassword = ({ type, isNew }) => {
  ...
  const handleSubmit = async () => {
    if (isNew) {
      if (isTermsService && createdBy !== "user") {
        if (!isTermsAggree) {
          toast.error(
            "You need to aggree with terms and conditions to proceed "
          );
          return;
        }
      }
      if (isPrivacyPolicy && createdBy !== "user") {
        if (!isPrivacyAggree) {
          toast.error(
            "You need to aggree with Komcrest privacy policy to proceed "
          );
          return;
        }
      }
  ...
  const [isConfirmPassowrdVisible, setIsConfirmPasswordVisible] =
    useState(false);

  const togglePasswordVisibility = () =>
    setIsPasswordVisible(!isPassowrdVisible);
  const toggleConfirmPasswordVisibility = () =>
    setIsConfirmPasswordVisible(!isConfirmPassowrdVisible);
    ...
    >
      <div className="w-[50%]">
        ...
        <Button
          ...
          className={`${
            isNew ? "mt-4" : "mt-7"
          } text-white px-8 text-base 2xl:text-[20px] bg-[#4fa82e]`}
          isDisabled={
            !formData?.password ||
      ...
      {isTermsServiceClick && (
        <TermsAndServicesModal
          ...
          data={modalData.termsOfServices}
          handleSubmit={() => handleAggree("terms")}
        />
      )}
      {isPrivacyPolicyClick && (
        <TermsAndServicesModal
          isOpen={isOpen}
          onOpenChange={onOpenChange}
          data={modalData.privacyPolicy}
          handleSubmit={() => handleAggree("privacy")}
        />
      )}
    </AuthLayout>
  );
